...
public class AuthorTabDAO {
    ...
    private AuthorTab convertRowToAuthorTab_Ratio(ResultSet myRs) throws SQLException {
    	String authorName = myRs.getString("AUTHOR_NAME");
    	String ratio = myRs.getString("DOLLAR_RATIO");
    ...
    public List<AuthorTab> searchAllAuthor(AuthorTabInput authorTabInput) {
    	...
    	if (authorTabInput.get_Selection() == null) {
    		...
    		} else if (authorTabInput.get_Selection() == "Book Type") {
    			
    		} else if (authorTabInput.get_Selection() == "Awarded In") {
    ...
    public List<AuthorTab> searchBestAuthor(AuthorTabInput authorTabInput) {
    	return null;
    }
    ...
    public List<AuthorTab> searchMostPublAuthor(AuthorTabInput authorTabInput) {
    	...
    	List<AuthorTab> authorTabList = new ArrayList<>();
    	if (authorTabInput.get_Selection() == null) {
    		query = "SELECT DISTINCT AUTHOR_NAME, NUM " +
    				"FROM (SELECT A.AUTHOR_NAME, COUNT(DISTINCT PUBL_ID) AS NUM " +
    		...
    		if (authorTabInput.get_Selection() == "In Language") {
    			String query1 = "SELECT DISTINCT AUTHOR_NAME, NUM " +
        						"FROM (SELECT A.AUTHOR_NAME, COUNT(DISTINCT PUBL_ID) AS NUM " +
        							  "FROM AUTHORS A, WRITTEN_PUBL_AUT W, " +
    ...
    public List<AuthorTab> searchMostWrittenAuthor(AuthorTabInput authorTabInput) {
    	...
    	if (authorTabInput.get_Selection() == null) {
    		query = "SELECT AUTHOR_NAME, NUM " +
    				"FROM (SELECT A.AUTHOR_NAME, COUNT(T.TITLE_ID) AS NUM " +
    		...
    		if (authorTabInput.get_Selection() == "In Language") {
    			String query1 = "SELECT AUTHOR_NAME, NUM " +
        						"FROM (SELECT A.AUTHOR_NAME, COUNT(T.TITLE_ID) AS NUM " +
        							  "FROM AUTHORS A, PUBLICATIONS P, WRITTEN_PUBL_AUT W, TITLES T, " +
